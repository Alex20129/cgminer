...
static void k_alloc_items(K_LIST *list, KLIST_FFL_ARGS)
{
	...
	if(list->is_store)
	{
		quithere(1, "List %s store can't %s()" KLIST_FFL, list->name, __func__, KLIST_FFL_PASS);
		return;
	}
	...
	if(!(list->item_memory=realloc(list->item_memory,
		...
		quithere(1, "List %s item_memory failed to realloc count=%d", list->name, list->item_mem_count);
		return;
	}
	item=calloc(allocate, sizeof(*item));
	if(!item)
	{
		quithere(1, "List %s failed to calloc %d new items - total was %d, limit was %d", list->name, allocate, list->total, list->limit);
		return;
	}
	...
	{
		list->data_mem_count++;
		if(!(list->data_memory=realloc(list->data_memory, list->data_mem_count * sizeof(*(list->data_memory)))))
		{
			quithere(1, "List %s data_memory failed to realloc count=%d", list->name, list->data_mem_count);
			return;
		}
...
void _k_insert_after(K_LIST *list, K_ITEM *item, K_ITEM *after, KLIST_FFL_ARGS)
{
	if(item->name != list->name)
	{
		quithere(1, "List %s can't %s() a %s item" KLIST_FFL, list->name, __func__, item->name, KLIST_FFL_PASS);
		return;
	}

	if(!after)
	{
		quithere(1, "%s() (%s) can't after a null item" KLIST_FFL, __func__, list->name, KLIST_FFL_PASS);
		return;
	}
