	...
	case AVA9_P_STATUS_ASIC:
		{
			...
			applog(LOG_DEBUG, "%s-%d-%d: AVA9_P_STATUS_ASIC %d-%d",
                        avalon9->drv->name, avalon9->device_id, modular_id, miner_id, asic_id);
...
static void avalon9_stratum_pkgs(struct cgpu_info *avalon9, struct pool *pool)
{
	...
	if(pool->stratum_diff <= AVA9_DRV_DIFFMAX)
		set_target(target, pool->stratum_diff);
	else
...
static inline void avalon9_detect(bool hotplug)
{
...
static int copy_pool_stratum(struct pool *pool_stratum, struct pool *pool)
{
	...
	pool_stratum->stratum_diff=pool->stratum_diff;
...
static void avalon9_set_asic_otp(struct cgpu_info *avalon9, int addr, unsigned int asic[])
{
	...
	applog(LOG_DEBUG, "%s-%d-%d: avalon9 set asic for otp reading %d, (%d-%d)", avalon9->drv->name, avalon9->device_id, addr, i, asic[0], asic[info->miner_count[addr] - 1]);
...
static void avalon9_set_freq(struct cgpu_info *avalon9, int addr, int miner_id, int asic_id, unsigned int freq[])
{
	...
	applog(LOG_DEBUG, "%s-%d-%d: avalon9 set freq miner %x-%x", avalon9->drv->name, avalon9->device_id, addr, miner_id, be32toh(tmp));
...
char *set_avalon9_device_freq(struct cgpu_info *avalon9, char *arg)
{
	...
	applog(LOG_NOTICE, "%s-%d:%d:%d:%d: Update frequency to %d", avalon9->drv->name, avalon9->device_id, val[0], val[1], val[2], val[3]);
	return NULL;
}
