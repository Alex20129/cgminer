...
#ifndef DRIVER_AVALON_7_H
#define DRIVER_AVALON_7_H
...
struct avalon7_info {
	...
	u_int8_t mm_dna[AVA7_DEFAULT_MODULARS][AVA7_MM_DNA_LEN];
	char mm_version[AVA7_DEFAULT_MODULARS][AVA7_MM_VER_LEN + 1]; /* It's a string */
	u_int32_t total_asics[AVA7_DEFAULT_MODULARS];
	u_int32_t max_ntime; /* Maximum: 7200 */

	int mod_type[AVA7_DEFAULT_MODULARS];
	u_int8_t miner_count[AVA7_DEFAULT_MODULARS];
	u_int8_t asic_count[AVA7_DEFAULT_MODULARS];

	u_int32_t freq_mode[AVA7_DEFAULT_MODULARS];
	...
	u_int8_t cutoff[AVA7_DEFAULT_MODULARS];
	...
	u_int32_t set_voltage[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT];
	u_int32_t set_frequency[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT][AVA7_DEFAULT_PLL_CNT];

	u_int16_t get_vin[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT];
	u_int32_t get_voltage[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT];
	u_int32_t get_pll[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT][AVA7_DEFAULT_PLL_CNT];
	/* spd_pass(4B), spd_fail(4B), sum_failed(4B), sum_num(4B), sum_xor(4B), PLL(6 * 4B) */
	u_int32_t get_asic[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT][AVA7_DEFAULT_ASIC_MAX][11];
	...
	u_int32_t error_code[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT + 1];
	u_int32_t error_crc[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_MINER_CNT];
	u_int8_t error_polling_cnt[AVA7_DEFAULT_MODULARS];

	u_int8_t power_good[AVA7_DEFAULT_MODULARS];
	char pmu_version[AVA7_DEFAULT_MODULARS][AVA7_DEFAULT_PMU_CNT][5];
	uint64_t diff1[AVA7_DEFAULT_MODULARS];

	u_int16_t vin_adc_ratio[AVA7_DEFAULT_MODULARS];
	u_int16_t vout_adc_ratio[AVA7_DEFAULT_MODULARS];
...
struct avalon7_dev_description {
	...
	int mod_type;
	u_int8_t miner_count; /* it should not greater than AVA7_DEFAULT_MINER_CNT */
	u_int8_t asic_count; /* asic count each miner, it should not great than AVA7_DEFAULT_ASIC_MAX */
	u_int16_t vin_adc_ratio;
	u_int16_t vout_adc_ratio;
	u_int32_t set_voltage;
};
...
extern u_int32_t opt_avalon7_th_pass;
extern u_int32_t opt_avalon7_th_fail;
extern u_int32_t opt_avalon7_th_init;
extern u_int32_t opt_avalon7_th_ms;
extern u_int32_t opt_avalon7_th_timeout;
extern u_int32_t opt_avalon7_nonce_mask;
extern bool opt_avalon7_asic_debug;
extern bool opt_avalon7_ssplus_enable;

#endif	/* DRIVER_AVALON_7_H */
