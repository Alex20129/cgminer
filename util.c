...
void __bin2hex(char *s, const uint8_t *p, size_t len)
{
	int i;
 ...
 * aligned array  sizes */
char *bin2hex(const uint8_t *p, size_t len)
{
	ssize_t slen;
	char *s;
...
bool hex2bin(uint8_t *p, const char *hexstr, size_t len)
{
	int nibble1, nibble2;
	uint8_t idx;
	bool ret=false;
	...
	while(*hexstr && len)
	{
		...
		*p++=(((uint8_t)nibble1) << 4) | ((uint8_t)nibble2);
		--len;
	}
...
static void decode_exit(struct pool *pool, char *b)
{
}
...
static bool parse_notify(struct pool *pool, json_t *val)
{
...
#endif
	char *job_id, *prev_hash, *coinbase1, *coinbase2, *bbversion, *nbit, *ntime, header[260];
	uint8_t *cb1=NULL, *cb2=NULL;
	...
	if(pool->next_diff > 0)
	{
		pool->stratum_diff=pool->next_diff;
	...
	if(merkles)
	{
		pool->swork.merkle_bin=cgrealloc(pool->swork.merkle_bin, sizeof(char *) * merkles + 1);
		if(!pool->swork.merkle_bin)
		{
			return(false);
		}
		for(i=0; i<merkles; i++)
...
static bool parse_extranonce(struct pool *pool, json_t *val)
{
	...
	free(pool->nonce1bin);
	pool->nonce1bin=(uint8_t *)calloc(pool->n1_len, 1);
	if((!pool->nonce1bin))
	{
...
{
	char *sockaddr_url, *stratum_port, *tmp, *url, *port;
	char address[256];
	int port_no;
...
static bool send_version(struct pool *pool, json_t *val)
{
	...
	sprintf(s, "{\"id\": %d, \"result\": \""PACKAGE_NAME"/"VERSION""STRATUM_USER_AGENT"\", \"error\": null}", id);
...
static bool parse_multi_version(struct pool *pool, json_t *params)
{
	int i;
	json_t *int_param;
	if(!params)
	{
		applog(LOG_ERR, "Pool %i No params given for parse_multi_version", pool->pool_no);
		return false;
	}
	if(!json_is_array(params))
	{
		applog(LOG_ERR, "Pool %i Invalid params given for parse_multi_version", pool->pool_no);
		return false;
	}
	for(i=0; i<json_array_size(params); i++)
	{
		int_param=json_array_get(params, i);
		if(!json_is_integer(int_param))
		{
			applog(LOG_ERR, "Pool %i Invalid param given for parse_multi_version", pool->pool_no);
			return false;
		}
		pool->version[i]=json_integer_value(int_param);
		applog(LOG_INFO, "Pool %i version[%i] has been set to 0x%X", pool->pool_no, i, pool->version[i]);
	}
	return true;
}

static bool parse_version_mask(struct pool *pool, json_t *params)
{
	uint32_t mask;
	...
	if(!params)
	{
		applog(LOG_ERR, "Pool %i No params given for parse_version_mask", pool->pool_no);
		return false;
	}

	if(!json_is_array(params))
	{
		applog(LOG_ERR, "Pool %i Invalid params given for parse_version_mask", pool->pool_no);
		return false;
	}

	str_param=json_array_get(params, 0);

	if(!json_is_string(str_param) || !json_string_length(str_param))
	{
		applog(LOG_ERR, "Pool %i Invalid string for parse_version_mask", pool->pool_no);
		return false;
	}

	version_mask_str=json_string_value(str_param);

	mask=strtol(version_mask_str, NULL, 16);
	if(!mask)
	{
		applog(LOG_ERR, "Pool %i invalid version mask 0x00000000", pool->pool_no);
		return false;
	}

	pool->version_mask=mask;

	applog(LOG_INFO, "Pool %i version mask has been set to 0x%X", pool->pool_no, pool->version_mask);
...
bool parse_method(struct pool *pool, char *s)
{
	...
	{
		ret=parse_multi_version(pool, params);
		pool->supports_version_rolling=ret;
		goto out_decref;
	}
	if(!strncasecmp(buf, "mining.set_version_mask", 23))
	{
		ret=parse_version_mask(pool, params);
		pool->supports_version_rolling=ret;
		goto out_decref;
	}
#endif
...
bool subscribe_extranonce(struct pool *pool)
 {
	...
	if(!res_val || json_is_false(res_val) || !json_is_null(err_val))
	{
        char *ss;
		if(err_val)
		{
...
bool auth_stratum(struct pool *pool)
{
	...
	if(opt_suggest_diff)
	{
		sprintf(s, "{\"id\": %d, \"method\": \"mining.suggest_difficulty\", \"params\": [%d]}", swork_id++, opt_suggest_diff);
...
static bool http_negotiate(struct pool *pool, int sockd, bool http0)
{
	...
	else
	{
		snprintf(buf, 1024, "CONNECT %s:%s HTTP/1.1\r\nHost: %s:%s\r\n\r\n",
			pool->sockaddr_url, pool->stratum_port, pool->sockaddr_url, pool->stratum_port);
	}
	applog(LOG_DEBUG, "Sending proxy %s:%s - %s", pool->sockaddr_proxy_url, pool->sockaddr_proxy_port, buf);
	...
	buf[len]='\0';
	applog(LOG_DEBUG, "Received from proxy %s:%s - %s", pool->sockaddr_proxy_url, pool->sockaddr_proxy_port, buf);
	if(strcmp(buf, "HTTP/1.1 200") && strcmp(buf, "HTTP/1.0 200"))
	{
		applog(LOG_WARNING, "HTTP Error from proxy %s:%s - %s", pool->sockaddr_proxy_url, pool->sockaddr_proxy_port, buf);
		return false;
	}
	...
	applog(LOG_DEBUG, "Success negotiating with %s:%s HTTP proxy", pool->sockaddr_proxy_url, pool->sockaddr_proxy_port);
	return true;
}
...
static bool socks5_negotiate(struct pool *pool, int sockd)
{
	...
	applog(LOG_DEBUG, "Attempting to negotiate with %s:%s SOCKS5 proxy", pool->sockaddr_proxy_url, pool->sockaddr_proxy_port );
	send(sockd, buf, 3, 0);
	if(recv_byte(sockd) != 0x05 || recv_byte(sockd) != buf[2])
	{
		applog(LOG_WARNING, "Bad response from %s:%s SOCKS5 server", pool->sockaddr_proxy_url, pool->sockaddr_proxy_port );
		return false;
	}
...
static bool decode_version_mask(struct pool *pool, json_t *version_mask_json)
{
	...
	unsigned int i, bit_idx, component_idx;
	...
	applog(LOG_INFO, "Pool %d version mask changed to %s", pool->pool_no, version_mask_str);
	...
	if((version_mask & VERSION_BITS_S9) != VERSION_BITS_S9)
	{
		applog(LOG_ERR, "Received version mask (0x%08lx) doesn't satisfy hardware requirement (0x%08lx)",
						   version_mask, (unsigned long)VERSION_BITS_S9);
		return(false);
	}
...
static bool configure_stratum_mining(struct pool *pool)
{
	...
	snprintf(s, RBUFSIZE,
			...
			"[[\""STRATUM_VERSION_ROLLING_STRING"\"], "
			"{\""STRATUM_VERSION_ROLLING_STRING".min-bit-count\": %d, "
			"\""STRATUM_VERSION_ROLLING_STRING".mask\": \"%x\"}]}",
			 swork_id++, 16, 0x1FFFE000);
	...
	if(!socket_full(pool, DEFAULT_SOCKWAIT))
	{
		applog(LOG_DEBUG, "Timed out waiting for response");
		goto out;
	}
	...
	response=json_loads(response_str, 0, &jansson_err);
	free(response_str);

	res_val=json_object_get(response, "result");
	err_val=json_object_get(response, "error");

	if(!res_val || json_is_null(res_val) || !json_is_null(err_val))
	{
		char *ss;
	...
	{
		if(!version_mask_valid && !strncasecmp(key, STRATUM_VERSION_ROLLING_STRING ".mask", 20))
		{
			version_mask_valid=decode_version_mask(pool, value);
			version_rolling=true;
		}
		if(!version_rolling && !strncasecmp(key, STRATUM_VERSION_ROLLING_STRING, 15))
		{
			version_rolling=json_boolean_value(value);
		}
	}
	/* Valid configuration for now only requires enabled version rolling and valid bit mask */
	config_status=version_rolling && version_mask_valid;
	if(config_status)
	{
		pool->supports_version_rolling=true;
		applog(LOG_INFO, "Pool %i supports version rolling", pool->pool_no);
	}
...
resend:
	...
	else
	{
		if(pool->sessionid)
		{
			sprintf(s, "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\""PACKAGE_NAME"/"PACKAGE_VERSION"\", \"%s\"]}", swork_id++, pool->sessionid);
		}
		else
		{
			sprintf(s, "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\""PACKAGE_NAME"/"PACKAGE_VERSION"\"]}", swork_id++);
		}
	}
	if(__stratum_send(pool, s, strlen(s)) != SEND_OK)
	{
	...
	val=json_loads(sret, 0, &jansson_err);
	free(sret);
	if(!val)
	{
		applog(LOG_INFO, "JSON decode failed(%d): %s", jansson_err.line, jansson_err.text);
		goto out;
	}
	...
	{
		applog(LOG_NOTICE, "Pool[%i] successfully negotiated resume with the same session ID", pool->pool_no);
	}
	...
	if(pool->sessionid)
	{
		applog(LOG_DEBUG, "Pool[%i] stratum session id: %s", pool->pool_no, pool->sessionid);
	}
...
//	}
	...
	if(ret)
	{
		...
		if(opt_protocol)
		{
			applog(LOG_DEBUG, "Pool[%i] confirmed mining.subscribe with extranonce1 %s extran2size %d", pool->pool_no, pool->nonce1, pool->n2size);
		}
	}
	else
	{
...
{
	int ret=sem_init(cgsem, 0, 0);
	if(ret)
	{
		quithere(1, "Failed to sem_init ret=%i errno=%i", ret, errno);
	}
}

void cgsem_post(cgsem_t *cgsem)
{
	int ret=sem_post(cgsem);
	if(ret)
	{
		quithere(1, "Failed to sem_post ret=%i errno=%i", ret, errno);
	}
}
...
void cgsem_wait(cgsem_t *cgsem)
{
	...
	while(ret)
	{
		if(!interrupted())
		{
			quithere(1, "Failed to sem_wait ret=%i errno=%i", ret, errno);
		}
		ret=sem_wait(cgsem);
	}
}
...
retry:
	...
	if(ret)
	{
		...
		quithere(1, "Failed to sem_timedwait ret=%i errno=%i", ret, errno);
	}
	return 0;
}
...
bool cg_completion_timeout(void (*fn)(void *fnarg), void *fnarg, int timeout)
{
	...
	cgc->fn=fn;
	cgc->fnarg=fnarg;
	pthread_create(&pthread, NULL, completion_thread, cgc);
