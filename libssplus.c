...
struct ssp_hashtable
{
	...
	u_int32_t size;
	u_int32_t max_size;  /* must be powers of 2 */
	u_int32_t limit;  /* probing limit */
	u_int32_t c1;
	u_int32_t c2;
};
...
#ifdef SORTER_DEBUG
static u_int32_t pair_count = 0;
static u_int32_t consumed = 0;
static u_int32_t discarded = 0;
static u_int32_t calls = 0;
static struct timeval ssp_ti, ssp_tf;
static double insert_time = .0;

static u_int32_t maxnonce = 0;
static u_int32_t ver = 0;
#endif
...
{
	u_int32_t last_nonce2 = 0, point_index = 0, nonce2;
...
static inline void ssp_hasher_fill_iram(struct ssp_hasher_instruction *p_inst, u_int32_t inst_index)
{
	u_int8_t i;
	volatile u_int32_t *p_iram_addr;
	u_int32_t tmp;
...
int ssp_hasher_init(void)
{
	...
	sspinfo.iram_addr = (volatile u_int32_t *)mmap(NULL, INSTRUCTIONS_RAM_SIZE,
...
void ssp_hasher_update_stratum(struct pool *pool, bool clean)
{
	...
	u_int32_t coinbase_len_posthash, coinbase_len_prehash;
	u_int32_t i, len_rem, block_nb;
	int merkle_index;
	u_int32_t inst_index = 0, nonce2_init = 0;
...
u_int32_t gen_merkle_root(struct pool *pool, uint64_t nonce2)
{
    ...
    data32 = (u_int32_t *)merkle_sha;
    swap32 = (u_int32_t *)merkle_root;
...

    {
        ...
        applog(LOG_DEBUG, "[M-N2]: %s-%08x-%08x", merkle_hash, (u_int32_t)nonce2le, (u_int32_t)nonce2);
        free(merkle_hash);
    }
