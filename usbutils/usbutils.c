...
static void add_in_use(uint8_t bus_number, uint8_t device_address, bool blacklist)
{
	...
	mutex_lock(&cgusb_lock);
	if ((!blacklist && __is_in_use(bus_number, device_address))) {
		found = true;
		goto nofway;
	}
	if (blacklist)
...
static struct usb_find_devices *usb_check(struct device_drv *drv, struct libusb_device *dev)
{
...
usb_perform_transfer(struct cgpu_info *cgpu, struct cg_usb_device *usbdev, int intinfo,
		  ...
		  unsigned int timeout, int mode, enum usb_cmds cmd,
		  int seq, bool cancellable, bool tt)
{
...
int __usb_transfer(struct cgpu_info *cgpu, uint8_t request_type, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, uint32_t *data, int siz, unsigned int timeout, enum usb_cmds cmd)
{
	struct cg_usb_device *usbdev;
#if DO_USB_STATS
	struct timeval tv_start, tv_finish;
