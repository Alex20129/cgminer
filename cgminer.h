...
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	DRIVER_ADD_COMMAND(antminer_s21)
#endif

#if defined(USE_S19XPH_DRIVER)
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	DRIVER_ADD_COMMAND(antminer_s19xph)
#endif

#if defined(USE_ANTMINER_L3)
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	DRIVER_ADD_COMMAND(antminer_l3)
#endif
...
#if defined(USE_AVALON_2)
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	DRIVER_ADD_COMMAND(avalon2)
#endif

#if defined(USE_AVALON_4)
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	DRIVER_ADD_COMMAND(avalon4)
#endif

#if defined(USE_AVALON_7)
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	DRIVER_ADD_COMMAND(avalon7)
#endif

#if defined(USE_AVALON_8)
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	DRIVER_ADD_COMMAND(avalon8)
#endif

#if defined(USE_AVALON_9)
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
...
struct cgminer_pool_stats
{
	...
	struct timeval last_share_result_lag;
...
static inline void wr_lock(pthread_rwlock_t *lock)
{
	...
	{
		quithere(1, "WTF WRLOCK ERROR ON LOCK! errno=%d", errno);
	}
	GOTLOCK(lock, file, func, line);
}

static inline int wr_trylock(pthread_rwlock_t *lock)
{
...
static inline void rd_lock(pthread_rwlock_t *lock)
{
	...
	{
		quithere(1, "WTF RDLOCK ERROR ON LOCK! errno=%d", errno);
	}
	GOTLOCK(lock, file, func, line);
}
...
static inline void _rw_unlock(pthread_rwlock_t *lock, const char *file, const char *func, const int line)
{
	...
	{
		quithere(1, "WTF RWLOCK ERROR ON UNLOCK! errno=%d", errno);
	}
	GUNLOCK(lock, file, func, line);
}
...
static inline void _mutex_init(pthread_mutex_t *lock, const char *file, const char *func, const int line)
{
	...
	{
		quithere(1, "Failed to pthread_mutex_init errno=%d", errno);
	}
...
static inline void _rwlock_init(pthread_rwlock_t *lock, const char *file, const char *func, const int line)
{
	...
	{
		quithere(1, "Failed to pthread_rwlock_init errno=%d", errno);
	}
...
extern json_t *config_json_data;
...
extern void __bin2hex(char *s, const uint8_t *p, size_t len);
extern char *bin2hex(const uint8_t *p, size_t len);
extern bool hex2bin(uint8_t *p, const char *hexstr, size_t len);
extern int max_scantime;
...
extern void gen_hash(uint8_t *data, uint8_t *hash, int len);
...
extern double g_hr_rolling1m, g_hr_rolling5m, g_hr_rolling15m;
extern double total_ghashes_done;
...
extern int64_t total_diff_accepted, total_diff_rejected, total_diff_stale;
extern unsigned int local_work;
extern unsigned int total_go, total_ro;
extern int opt_cutofftemp;
extern int opt_log_interval;
extern char current_hash[68];
extern int64_t current_diff;
extern int64_t best_diff;
...
struct pool
{
	...
	int transactions;
	char *txn_data;
	uint8_t scriptsig_base[100];
	uint8_t script_pubkey[25 + 3];
	int nValue;
...
struct work
{
	uint8_t	data[128];
#if defined(ENABLE_ASIC_BOOST)
	uint8_t	midstate[MIDSTATE_NUM][32];
#else
	uint8_t	midstate[32];
#endif
	uint8_t	target[32];
...
extern struct work *get_work(struct thr_info *thr);
...
extern void hash_driver_work(struct thr_info *thr);
extern void hash_queued_work(struct thr_info *thr);
extern void _wlog(const char *str);
extern void _wlogprint(const char *str);
extern void kill_work(void);
extern void switch_pools(struct pool *selected);
extern void discard_work(struct work **workptr);
...
extern void free_work(struct work **workptr);
...
extern struct api_data *api_add_int_array(struct api_data *root, char *name, int *data, bool copy_data);
extern struct api_data *api_add_int(struct api_data *root, char *name, int *data, bool copy_data);
extern struct api_data *api_add_int8(struct api_data *root, char *name, int8_t *data, bool copy_data);
extern struct api_data *api_add_int16(struct api_data *root, char *name, int16_t *data, bool copy_data);
extern struct api_data *api_add_int32(struct api_data *root, char *name, int32_t *data, bool copy_data);
extern struct api_data *api_add_int64(struct api_data *root, char *name, int64_t *data, bool copy_data);
extern struct api_data *api_add_uint_array(struct api_data *root, char *name, uint *data, bool copy_data);
extern struct api_data *api_add_uint(struct api_data *root, char *name, uint *data, bool copy_data);
...
extern struct api_data *api_add_float_array(struct api_data *root, char *name, float *data, bool copy_data);
