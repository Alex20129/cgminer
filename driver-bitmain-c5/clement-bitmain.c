...
extern int get_nonce_number_in_fifo();
extern int get_return_nonce(unsigned int *buf);
extern int get_nonce_fifo_interrupt();
    ...
    if((Conf.StartSensor > 0) && (cgpu.real_asic_num != 1))
    {
        ...
        for(i=0; i<loop; i++)
        {
			...
			if(opened_core_num >= Conf.CoreNum-chain_badcore_num[chainIndex][i])
				...
				last_result_opencore[chainIndex][i]=0;
	        ...
	        while(which_asic[chainIndex] < CHAIN_ASIC_NUM)
	        {
	            ...
	            if(work_fifo_ready & (0x1 << chainIndex))   // work fifo is not full, we can send work
	            {
	                ...
	                else    // vil mode
	                {
	                    if(ASIC_TYPE==1387)
	                    {
							memset(buf_vil, 0x0, sizeof(buf_vil));
...
bool clement_doTestBoardOnce(bool showlog, FILE* file)
{
	...
	for(i=0; i<BITMAIN_MAX_CHAIN_NUM; i++)
	{
        ...
        if(file)
        {
            ...
            fwrite(&dt, sizeof(dt), 1, file);
            
            dt=(short)cgpu.real_asic_num;
            fwrite(&dt, sizeof(dt), 1, file);
            ...
            for(i=0; i<BITMAIN_MAX_CHAIN_NUM; i++)
            {
                if(cgpu.chain_exist[i])
                {
                    ...
                    for(j=0;j < cgpu.real_asic_num;j++)
                    {
                        dt=(short)asic_nonce_num[i][j];
                        fwrite(&dt, sizeof(dt), 1, file);
                    
                        dt=(short)getChainAsicFreqIndex(i, j);
                        fwrite(&dt, sizeof(dt), 1, file);
                    }
                }
            }
